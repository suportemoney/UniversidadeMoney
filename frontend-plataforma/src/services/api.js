...
export async function validarTokenAcesso(chave) {
  return apiFetch("/auth/token-acesso/validar/", {
    method: "POST",
    body: JSON.stringify({ chave: String(chave || "").trim().toUpperCase() }),
  });
}

export async function ativarTokenAcesso(chave, novaSenha, cpf) {
  return apiFetch("/auth/token-acesso/ativar/", {
    method: "POST",
    body: JSON.stringify({
