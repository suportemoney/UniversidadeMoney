...
docker compose -f compose.yml -f compose.vps.yml --env-file .env up -d db
```

## 3. Restore no container

```bash
docker compose -f compose.yml -f compose.vps.yml --env-file .env \
...
```bash
docker compose -f compose.yml -f compose.vps.yml --env-file .env up -d backend-prod
docker cp /var/www/universidade/media/. "$(docker compose -f compose.yml -f compose.vps.yml --env-file .env ps -q backend-prod)":/data/media/
```

Depois reinicie `backend-prod`.
